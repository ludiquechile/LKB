    ...
    ui(new Ui::sktinfoextractor)
{
    ...
    useDC=false;
    instance=this;
    num_frames_to_recal = ui->num_frames_to_Recal->value();
    last_recal = 0;
...
#pragma omp parallel private(tid,moms,b,pt) shared(update)
            ...
            if(cam->updateCam(false,0,true,false)){
                ...
                if(cConfig[0]->method=="Background"){
                    ...
                    last_recal++;
                    if(num_frames_to_recal>0 && last_recal>num_frames_to_recal){
                        //cout << "Reclibrated" << endl;
                    // TODO : PONER ACA EL BACKMETHOD SI PASA ALGO...
                        cConfig[0]->BM->start(2);
                        cConfig[0]->maskZona.setTo(cv::Scalar::all(0));
                        fillConvexPoly(cConfig[0]->maskZona,cConfig[0]->zone,4,cvScalar(255));
                         cout << cConfig[0]->BM->proc(cConfig[0]->variance,depthsS[0],masks[0],cConfig[0]->Background,cConfig[0]->maskBack);
                         cout << cConfig[0]->BM->proc(cConfig[0]->variance,depthsS[0],masks[0],cConfig[0]->Background,cConfig[0]->maskBack);
                         cout << cConfig[0]->BM->proc(cConfig[0]->variance,depthsS[0],masks[0],cConfig[0]->Background,cConfig[0]->maskBack);
                         cout << cConfig[0]->BM->proc(cConfig[0]->variance,depthsS[0],masks[0],cConfig[0]->Background,cConfig[0]->maskBack);
                         last_recal = 0;
...
void sktinfoextractor::on_num_frames_to_Recal_valueChanged(int arg1)
{
    num_frames_to_recal = arg1;
}

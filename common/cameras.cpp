...
cameras::cameras()
{
    ...
    for(int i=0;i<10;i++){
        ...
        retrievedDM[i]=false;
        depths[i].create(480,640,CV_16UC1);
    }
}
...
void cameras::retriveDepthAndMask(int num, Mat& dep,Mat& mask){
    if(num<numContextosIniciados){
        if(!retrievedDM[num]){
            memcpy( depths[num].data, depthMD[num].Data(), 640*480*sizeof(XnDepthPixel) );
            masks[num] = (depths[num] == noSampleValue) | (depths[num] == shadowValue) | (depths[num] == 0);
            depths[num].setTo( cv::Scalar::all( 0 ), masks[num] );
            masks[num] = (masks[num]==0);
            retrievedDM[num]=true;
        }
        dep = depths[num];
        mask = masks[num];
...
void cameras::retrivePointCloudFast(int num,Mat* dep,Mat* out){
    if(num<numContextosIniciados){
    int rows=480,cols=640;
    (*out).create(rows,cols,CV_32FC3); // MAYBE RESIZE???
    /// NEEDED? : out.setTo(cv::Scalar::all( 0 ));
